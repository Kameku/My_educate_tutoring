...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-auto">
            <a href="{{ route('concept.show', ['concept' => $concept->id]) }}" class="badge badge-secondary mb-2 p-2">Go Back</a>
        </div>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-auto">
            <button class="btn btn-primary px-8" data-toggle="modal" data-target="#createLearning">Create Learning Activity</button>
        </div>
    </div>

    <div class="row mb-3">
        <div class="col-12">
            <p class="alert alert-primary">{{$subject->name_sub.' / '.$concept->concept_name.' / '.$conceptDetail->concept_detail_name}}</p>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
                    ...
                    {{-- <div class="table-responsive">
                        <table class="table table-nowrap table-hover table-centered mb-0">
                            ...
                            <tbody class="border-none ">
                                ...
                                <tr>
                                    <td>
                                        <span class="text-dark" style="text-transform:uppercase;">{{$learning->learning_name}}</span>
                                    </td>
                                    <td style="width: 50px">
                                        <a href="{{route('concept.show', ['concept' => $concept->id])}}" class="btn btn-sm btn-info">View</a>
                                    </td>
                                    <td style="width: 50px">
                                        <form action="{{route('learning.destroy', ['learning' => $learning->id])}}" method="POST">
                                            @method('DELETE') 
                                            @csrf 
                                ...
                                <tbody>
                                    @foreach ($learnings as $learning)
                                        <tr>
                                            ...
                                            <td style="width: 50px">
                                                <a class="btn btn-primary btn-sm text-white btn-block" data-toggle="modal" data-target="#learningEdit{{$learning->id}}">Edit</a> 
                                                @include('library.learningEditModal') 
                                            </td>
                                            <td style="width: 50px">
                                                <form action="{{route('learning.destroy', ['learning' => $learning->id])}}" method="POST">
                                                    @method('DELETE') 
    ...
    {{-- @include('library.conceptDetailsModal') --}}
    @include('library.learningModal')
