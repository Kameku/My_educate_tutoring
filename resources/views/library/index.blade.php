...
@section('content')
    ...
    <div class="row mb-3">
        ...
        <div class="col-auto">
            <button class="btn btn-primary px-8" data-toggle="modal" data-target="#createSubjects">Create Subject</button>
        </div>
    </div>
                        ...
                        aria-describedby="datatable-buttons_info">
                            ...
                            <tbody>
                                ...
                                <tr role="row" class="odd">
                                    <td><span class="text-dark" style="text-transform:uppercase;">{{ $library->name_sub }}</span></td>
                                    <td style="width: 50px">
                                        <a href="{{route('library.show', ['library' => $library->id])}}" class="btn btn-sm btn-info btn-block">View</a>
                                    </td>
                                    <td style="width: 50px">
                                        {{-- <a href="{{route('library.update', ['library'=>$library->id])}}" class="btn btn-sm btn-primary">Update</a> --}}
                                        <a class="btn btn-primary btn-sm text-white btn-block" data-toggle="modal" data-target="#SubjectEdit{{$library->id}}">Edit</a> 
                                        @include('library.subjectEditModal') 
                                    </td>
                                    <td style="width: 50px">
                                        @if (count($library->concepts) > 0)
                                            <span class="btn btn-sm btn-secondary btn-block" disabled>Delete</span>
                                        @else
                                            <form action="{{route('library.destroy', ['library' => $library->id])}}" method="POST">
                                                @method('DELETE') 
                                                @csrf 
